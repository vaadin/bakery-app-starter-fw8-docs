...
https://bakery.demo.vaadin.com[See a live demo of the application.]
...
A personalized project can be downloaded from the https://vaadin.com/full-stack-starter[product page] by giving group id, artifact id and developer name.

A paid Pro or Prime subscription is required for creating a new software project from a Starter template. After its creation, results can be used, developed and distributed freely, but licenses for the used commercial components are required during development. The Starter or its parts cannot be redistributed as a code example or template. For full terms, see https://vaadin.com/license/cvtl-1[Commercial Vaadin Template License].
